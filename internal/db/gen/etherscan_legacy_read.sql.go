...
// versions:
//   sqlc v1.31.1
// source: etherscan_legacy_read.sql
...
type EtherscanBlockCountdownRow struct {
	CurrentSampleNumber     string `db:"current_sample_number" json:"current_sample_number"`
	CurrentSampleTimestamp  string `db:"current_sample_timestamp" json:"current_sample_timestamp"`
	AnchorNumber            string `db:"anchor_number" json:"anchor_number"`
	AnchorTimestamp         string `db:"anchor_timestamp" json:"anchor_timestamp"`
	SampleCountValue        string `db:"sample_count_value" json:"sample_count_value"`
	CoverageConfiguredStart string `db:"coverage_configured_start" json:"coverage_configured_start"`
...
type EtherscanContractCreationRow struct {
	...
	TraceDepth      *int32  `db:"trace_depth" json:"trace_depth"`
	CallType        *string `db:"call_type" json:"call_type"`
	FactoryAddress  []byte  `db:"factory_address" json:"factory_address"`
...
type EtherscanTransactionStatusRow struct {
	Raw                []byte `db:"raw" json:"raw"`
	TxHash             []byte `db:"tx_hash" json:"tx_hash"`
	BlockHash          []byte `db:"block_hash" json:"block_hash"`
	ReceiptBlockNumber string `db:"receipt_block_number" json:"receipt_block_number"`
	TxIndex            int64  `db:"tx_index" json:"tx_index"`
}
...
, current_binding AS (
    SELECT binding.chain_id, binding.proxy_address, binding.proxy_code_hash, binding.observation_block_number, binding.observation_block_hash, binding.observation_stage_version, binding.proxy_kind, binding.proxy_pattern, binding.standard_version, binding.implementation_address, binding.implementation_code_hash, binding.admin_address, binding.admin_code_hash, binding.beacon_address, binding.beacon_code_hash, binding.management_kind, binding.management_address, binding.management_code_hash, binding.observation_generation_id, binding.artifact_resolution_id, binding.beacon_generation_id, binding.context_block_number, binding.context_block_hash, binding.verification_job_id, binding.request_digest, binding.created_at, binding.uups_generation_id, current_proxy.context_number,
           current_proxy.proxy_artifact_job_id,
           current_proxy.implementation_artifact_job_id
    FROM current_proxy
    JOIN verified_proxy_bindings AS binding
