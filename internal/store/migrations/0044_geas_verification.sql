-- P63 adds one statically linked Geas compiler family. Solidity/Yul continue
-- to bind an immutable catalog generation; Geas binds only its Go module and
-- helper executable identities.

DROP TRIGGER verification_jobs_solcjs_compiler_provenance ON verification_jobs;
DROP TRIGGER verification_jobs_unbound_solcjs_insert ON verification_jobs;
DROP FUNCTION enforce_solcjs_compiler_provenance();
DROP FUNCTION enforce_unbound_solcjs_job_insert();
...
ALTER TABLE verification_jobs
    ADD CONSTRAINT verification_jobs_language_check
        CHECK (language IS NULL OR language IN ('solidity', 'yul', 'geas')),
    ADD CONSTRAINT verification_jobs_compiler_check CHECK (
        (
            ...
            kind = 'address' AND language = 'geas' AND
            compiler_version = '0.3.3' AND catalog_language IS NULL AND
            (
                (
                    compiler_platform IS NULL AND
                    catalog_generation_id IS NULL AND
                    ...
                    compiler_platform = 'go-module' AND
                    catalog_generation_id IS NULL AND
                    octet_length(compiler_digest) = 32 AND
                    executor_kind = 'etherview_geas_v1' AND
                    execution_policy = 'trusted_subprocess' AND
                    octet_length(executor_digest) = 32
                )
            )
...
BEGIN
    IF jsonb_typeof(candidate) IS DISTINCT FROM 'object' OR
       candidate = '{}'::jsonb THEN
        RETURN FALSE;
    END IF;
    RETURN NOT EXISTS (
        SELECT 1
        FROM jsonb_each(candidate) AS source(name, value)
        WHERE CASE
            WHEN jsonb_typeof(source.value) = 'object' THEN
                jsonb_typeof(source.value->'content') IS DISTINCT FROM 'string' OR
                source.value - 'content' <> '{}'::jsonb
            ELSE TRUE
        END
    );
END
...
ALTER TABLE verification_results
    ...
    ADD CONSTRAINT verification_results_success_shape_check CHECK (
        ...
        (outcome_kind = 'verification_success' AND
            ...
            language IN ('solidity', 'yul', 'geas') AND
            length(compiler_version) BETWEEN 1 AND 128 AND
...
CREATE TRIGGER verification_jobs_compiler_provenance
BEFORE UPDATE OF
...
FOR EACH ROW EXECUTE FUNCTION enforce_verification_compiler_provenance();
...
CREATE TRIGGER verification_jobs_unbound_compiler_insert
BEFORE INSERT ON verification_jobs
FOR EACH ROW EXECUTE FUNCTION enforce_unbound_verification_job_insert();
...
BEGIN
    IF migration_schema IS NULL THEN
        RAISE EXCEPTION 'Geas verification migration requires a current schema';
    END IF;
    EXECUTE format(
    ...
    EXECUTE format(
        'ALTER FUNCTION %I.enforce_verification_compiler_provenance() SET search_path = %I, pg_catalog',
        migration_schema, migration_schema
    );
    EXECUTE format(
