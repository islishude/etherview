...
var (
	...
	workItemIDPattern = regexp.MustCompile(`^P[0-9]{2}-T[0-9]{2,3}$`)
	dependencyPattern = regexp.MustCompile(`P[0-9]{2}(?:-T[0-9]{2,3})?`)
	dependencyRange   = regexp.MustCompile(`(P[0-9]{2}(?:-T[0-9]{2,3})?)\s*[-\x{2013}\x{2014}]\s*(P[0-9]{2}(?:-T[0-9]{2,3})?)`)
	...
	for _, plan := range c.children {
		...
		for _, item := range plan.items {
			if !workItemIDPattern.MatchString(item.id) {
				c.add(plan.path, item.line, fmt.Sprintf("malformed work-item ID %q; expected PNN-TNN or PNN-TNNN", item.id))
