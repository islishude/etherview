...
var ErrLeaseLost = errors.New("verification job lease is no longer owned")
...
type Repository interface {
	Submit(context.Context, Request) (VerificationJob, bool, error)
...
func NewPostgresRepository(db *sql.DB, options RepositoryOptions) (*PostgresRepository, error) {
	...
	options.defaults()
	if options.MaxRequestBytes <= 0 || options.MaxResultBytes <= 0 {
...
func (repository *PostgresRepository) Submit(ctx context.Context, request Request) (VerificationJob, bool, error) {
	...
	if errors.Is(scanErr, sql.ErrNoRows) {
		job, scanErr = repository.scanJob(tx.QueryRowContext(ctx, selectVerificationBindingSQL,
			strconv.FormatUint(request.ChainID, 10), address, codeHash, blockHash,
		))
	}
	if scanErr != nil {
...
func (repository *PostgresRepository) Complete(ctx context.Context, lease VerificationLease, completion Completion) error {
	...
	if completion.Kind != MatchMismatch {
		if err := repository.publishVerifiedContract(ctx, tx, lease.Job, completion); err != nil {
			return err
		}
	}
	result, err := tx.ExecContext(ctx, completeVerificationSQL,
		lease.Job.ID, lease.Token, completion.Kind, string(encodedResult),
	)
	if err != nil {
...
func (repository *PostgresRepository) publishVerifiedContract(ctx context.Context, tx *sql.Tx, job VerificationJob, completion Completion) error {
	...
	if err := tx.QueryRowContext(ctx, verificationCanonicalTargetSQL,
		...
		return fmt.Errorf("resolve canonical verification target: %w", err)
	}
	blockNumber, err := strconv.ParseUint(blockNumberText, 10, 64)
	if err != nil || strconv.FormatUint(blockNumber, 10) != blockNumberText {
		return errors.New("resolve canonical verification target: invalid block number")
	}
...
func validatePersistedJobState(job VerificationJob) error {
	...
	case JobSucceeded:
		if job.ResultKind == nil || job.Result == nil || job.ErrorCode != "" ||
			!validMatchKind(*job.ResultKind) || *job.ResultKind != summarizeMatch(job.Result.Match) {
...
compiler_version, request, status, result_kind, result, error_code,
...
job.compiler_version, job.request, job.status, job.result_kind, job.result, job.error_code,
...
ON CONFLICT (chain_id, address, code_hash, block_hash) DO NOTHING
...
) VALUES ($1::numeric, $2, $3, $4::numeric, $5, $6, $7, $8, $9::jsonb, $10::jsonb, $11::jsonb)
...
WHERE verified_contracts.match_kind <> 'exact' OR EXCLUDED.match_kind = 'exact'`
