      ...
      {operation.data ? (
        <div className="page-stack user-operation-detail">
          ...
          <DetailList label={t("page.userOperation")}>
            <Detail
              label={t("userOperation.hash")}
              mono
              wide
              value={<HashValue value={operation.data.hash} />}
            />
            <Detail
              label={t("table.status")}
              value={<UserOperationStatus success={operation.data.success} />}
            />
            <Detail
              label={t("userOperation.version")}
              value={`v${operation.data.entry_point_version}`}
            />
            <Detail
              label={t("userOperation.entryPoint")}
              value={<AddressIdentity address={operation.data.entry_point} />}
            />
            <Detail
              label={t("userOperation.sender")}
              value={<AddressIdentity address={operation.data.sender} />}
            />
            <Detail
              label={t("userOperation.nonce")}
              value={formatInteger(operation.data.nonce, locale)}
            />
            <Detail
              label={t("userOperation.nonceKey")}
              value={formatInteger(operation.data.nonce_key, locale)}
            />
            <Detail
              label={t("userOperation.nonceSequence")}
              value={formatInteger(operation.data.nonce_sequence, locale)}
            />
            <Detail
              label={t("userOperation.bundle")}
              mono
              value={
                <Link
                  to="/tx/$hash"
                  params={{ hash: operation.data.transaction_hash }}
                  search={{ tab: "user-operations" }}
                >
            ...
            />
            <Detail
              label={t("userOperation.operationIndex")}
              value={formatInteger(operation.data.operation_index, locale)}
            />
            <Detail
              label={t("userOperation.eventLogIndex")}
              value={formatInteger(operation.data.event_log_index, locale)}
            />
            <Detail
              label={t("table.block")}
              value={
            ...
            <Detail
              ...
              value={formatTimestamp(operation.data.block_timestamp, locale)}
            />
            <Detail
              label={t("table.finality")}
              value={<FinalityBadge finality={operation.data.finality} />}
            />
            <Detail
              label={t("userOperation.bundler")}
              value={<AddressIdentity address={operation.data.bundler} />}
            />
            <Detail
              label={t("userOperation.beneficiary")}
              value={<AddressIdentity address={operation.data.beneficiary} />}
            />
            <Detail
              label={t("userOperation.initialization")}
              value={t(`userOperation.initKind.${operation.data.init_kind}`)}
            />
            <Detail
              label={t("userOperation.factory")}
              value={
                operation.data.factory ? <AddressIdentity address={operation.data.factory} /> : "—"
              }
            />
            <Detail
              label={t("userOperation.paymaster")}
              value={
                operation.data.paymaster ? (
                  <AddressIdentity address={operation.data.paymaster} />
                ) : (
                  "—"
                )
              }
            />
            <Detail
              label={t("userOperation.aggregator")}
              value={
                operation.data.aggregator ? (
                  <AddressIdentity address={operation.data.aggregator} />
                ) : (
                  "—"
                )
              }
            />
            <Detail
              label={t("userOperation.gasUsed")}
              value={formatInteger(operation.data.actual_gas_used, locale)}
            />
            <Detail
              label={`${t("userOperation.gasCost")} (${nativeSymbol})`}
              value={
                <code>
                  {formatNativeAmount(operation.data.actual_gas_cost, locale, nativeDecimals)}
                </code>
              }
            />
          </DetailList>
...
function UserOperationFailure({ operation }: { operation: UserOperationDetail }) {
  ...
  return (
    <section className="reorg-context user-operation-failure" role="status">
      ...
      <div>
        <h2>
          {failure ? t(`userOperation.eventKind.${failure.kind}`) : t("userOperation.failed")}
        </h2>
...
function UserOperationRequestPanel({ operation }: { operation: UserOperationDetail }) {
  ...
  return (
    <DetailList label={t("userOperation.request")}>
      <Detail
        label={t("userOperation.callGasLimit")}
        value={formatInteger(request.call_gas_limit, locale)}
      />
      <Detail
        label={t("userOperation.verificationGasLimit")}
        value={formatInteger(request.verification_gas_limit, locale)}
      />
      <Detail
        label={t("userOperation.preVerificationGas")}
        value={formatInteger(request.pre_verification_gas, locale)}
      />
      <Detail
        label={t("userOperation.maxFeePerGas")}
        value={formatInteger(request.max_fee_per_gas, locale)}
      />
      <Detail
        label={t("userOperation.maxPriorityFeePerGas")}
        value={formatInteger(request.max_priority_fee_per_gas, locale)}
      />
      <Detail
        label={t("userOperation.paymasterVerificationGasLimit")}
        value={formatInteger(request.paymaster_verification_gas_limit, locale)}
      />
      <Detail
        label={t("userOperation.paymasterPostOpGasLimit")}
        value={formatInteger(request.paymaster_post_op_gas_limit, locale)}
      />
      {request.account_gas_limits ? (
        <RawDetail label={t("userOperation.accountGasLimits")} value={request.account_gas_limits} />
      ) : null}
      {request.gas_fees ? (
        <RawDetail label={t("userOperation.gasFees")} value={request.gas_fees} />
      ) : null}
      ...
      <RawDetail
        label={t("userOperation.paymasterSignature")}
        value={request.paymaster_signature}
      />
      <RawDetail label={t("userOperation.signature")} value={request.signature} />
      <RawDetail
        label={t("userOperation.aggregatedSignature")}
        value={request.aggregated_signature}
      />
    </DetailList>
  );
}
  ...
  return (
    <section className="panel detail-card" aria-labelledby="user-operation-events-title">
      ...
      {operation.events.length === 0 ? (
        <p className="empty-result">{t("userOperation.noEvents")}</p>
      ) : null}
        ...
        {operation.events.map((event) => (
          <article className="transaction-log" key={`${event.log_index}:${event.kind}`}>
            ...
            <dl className="detail-grid compact-grid">
              ...
              <Detail
                label={t("userOperation.sender")}
                value={<AddressIdentity address={event.sender} />}
              />
              <Detail label={t("userOperation.nonce")} value={formatInteger(event.nonce, locale)} />
              <Detail
                label={t("detail.address")}
                value={
                  event.related_address ? <AddressIdentity address={event.related_address} /> : "—"
                }
              />
              <Detail
                label={t("userOperation.paymaster")}
                value={event.paymaster ? <AddressIdentity address={event.paymaster} /> : "—"}
              />
              {event.reason ? (
                <Detail label={t("detail.reason")} value={event.reason} wide />
              ) : null}
