...
describe("core explorer pages", () => {
  ...
  it("polls a basic mempool detail through pending, replaced, and included states", async () => {
    ...
    expect(
      requested.some((path) => path.startsWith(`/api/v1/transactions/${transactionHash}/`)),
    ).toBe(false);
    ...
    expect(
      document.querySelector('[data-status="replaced"] svg.lucide-arrow-right-left'),
    ).not.toBeNull();
    expect(
      requested.some((path) => path.startsWith(`/api/v1/transactions/${transactionHash}/`)),
    ).toBe(false);
    ...
    expect(
      document.querySelector('[data-status="success"] svg.lucide-circle-check'),
    ).not.toBeNull();
...
describe("transaction navigation and calldata", () => {
  ...
  it.each([
    ...
    "classifies $name from transaction-time execution code",
    async ({ input, value, resolution, english, chinese }) => {
      const requested: string[] = [];
      vi.stubGlobal(
        "fetch",
  ...
  it("classifies contract creation without requesting execution evidence", async () => {
    ...
    renderExplorer(`/tx/${transactionHash}`);
    const action = (await screen.findByText("Transaction action", { exact: true })).closest(
    ...
    await userEvent.setup().click(screen.getByRole("button", { name: "切换到中文" }));
    await waitFor(() =>
      expect(within(action).getByText("创建合约", { exact: true })).toBeVisible(),
    );
  });
  ...
  it("shows a fail-closed loading action without briefly inferring a contract call", async () => {
    ...
    renderExplorer(`/tx/${transactionHash}`);
    expect(
      await screen.findByText("Determining transaction action", { exact: true }),
    ).toBeVisible();
  ...
  it.each(["request failure", "unavailable execution identity"] as const)(
    ...
    async (mode) => {
      ...
      renderExplorer(`/tx/${transactionHash}`);
      expect(
        await screen.findByText("Transaction action unavailable", { exact: true }),
      ).toBeVisible();
      expect(screen.queryByText("Contract interaction", { exact: true })).not.toBeInTheDocument();
  ...
  it("renders named structs and nested arrays as a bounded keyboard tree in both locales", async () => {
              ...
              inputs: [
                {
                  ...
                  components: [
                    { name: "owner", type: "address", components: [] },
                    {
                      name: "rules",
                      type: "tuple[]",
                      internal_type: "struct Fixture.Rule[]",
                      components: [
    ...
    expect(
      within(decoded).queryByText(JSON.stringify(nestedValue), { exact: true }),
    ...
    expect(screen.getByRole("textbox", { name: "原始 calldata（十六进制）" })).toHaveValue(
      calldata,
    );
    expect(requested).not.toContain(`/api/v1/contracts/${address}/verification`);
    expect(requested).not.toContain(`/api/v1/contracts/${address}/proxy`);
  ...
  it("keeps raw calldata when a decoded parameter response is structurally inconsistent", async () => {
    ...
    expect(
      await screen.findByText(
        "Decoded parameter structure is unavailable. Raw calldata remains available below.",
      ),
    ).toBeVisible();
  ...
  it("classifies an exact-address decoded call when execution identity is unavailable", async () => {
    ...
    expect(await screen.findByText("Contract interaction", { exact: true })).toBeVisible();
    expect(
      screen.queryByText("Transaction action unavailable", { exact: true }),
    ).not.toBeInTheDocument();
    await userEvent.setup().click(await screen.findByText("More details"));
    expect(
