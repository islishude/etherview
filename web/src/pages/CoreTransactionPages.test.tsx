...
describe("core transaction detail pages", () => {
  it("decodes an ordinary call from its transaction-time EIP-7702 delegate", async () => {
      ...
      vi.fn(async (input: RequestInfo | URL) => {
        ...
        if (url.pathname === `/api/v1/transactions/${transactionHash}/calldata`) {
          return envelope({
            ...
            input: calldata,
            execution: {
              context_address: delegatedAddress,
            ...
            decoding: {
              ...
              inputs: [{ name: "value", type: "uint256", value: "42", components: [] }],
              candidates: [],
              confidence: "verified",
              abi_source: {
                kind: "exact_address",
    ...
    const decoded = await screen.findByRole("region", {
      name: "Decoded calldata · setValue(uint256)",
    });
  ...
  it("keeps raw calldata and reports no executable code after a type-4 clearing authorization", async () => {
    ...
    await userEvent.setup().click(await screen.findByText("More details"));
    expect(
      await screen.findByText(/No executable code at transaction execution time/u),
    ).toBeVisible();
  ...
  it("renders topics and data directly in collapsed details and converts anonymous topic zero", async () => {
            ...
            items: [
              {
                address,
                log_index: "0",
                topics: [eventTopic, addressTopic, textTopic, numberTopic],
                data: "0x1234",
                decoding: {
                  status: "decoded",
                  event_name: "Changed",
                  signature: "Changed(tuple,uint256[],address,string,uint256)",
                  confidence: "verified",
              ...
              },
              {
                address,
                log_index: "1",
                topics: [addressTopic, textTopic, numberTopic],
                data: "0xabcd",
                decoding: {
                  status: "decoded",
                  event_name: "AnonymousChanged",
                  signature: "AnonymousChanged(address,string,uint256)",
                  confidence: "verified",
                  abi_source: { kind: "exact_address", address, code_hash: canonicalHash },
                  arguments: [
                    {
    ...
    expect(
      await screen.findByText("Changed(tuple,uint256[],address,string,uint256)"),
    ).toBeVisible();
    ...
    expect(
      within(argumentsTable).getAllByRole("button", { name: "Copy" }).length,
    ...
    expect(
      within(card).queryByRole("combobox", { name: "Topic 0 display mode" }),
    ...
    expect(
      within(card).getByText("0x52908400098527886E0F7030069857D2E4169EE7", { exact: true }),
    ).toBeVisible();
    await user.selectOptions(
      within(card).getByRole("combobox", { name: "Topic 2 display mode" }),
      "text",
    );
    ...
    const addressTopicRow = within(card)
      .getByRole("combobox", { name: "Topic 1 display mode" })
      .closest<HTMLElement>(".transaction-topic");
    ...
    const anonymousTopicZeroMode = within(anonymousCard).getByRole("combobox", {
      name: "Topic 0 display mode",
    });
    await user.selectOptions(anonymousTopicZeroMode, "address");
    expect(
      within(anonymousCard).getByText("0x52908400098527886E0F7030069857D2E4169EE7", {
        exact: true,
      }),
    ).toBeVisible();
    await user.selectOptions(
      within(anonymousCard).getByRole("combobox", { name: "Topic 1 display mode" }),
      "text",
    );
    expect(within(anonymousCard).getByText("hello", { exact: true })).toBeVisible();
    const anonymousNumberMode = within(anonymousCard).getByRole("combobox", {
      name: "Topic 2 display mode",
    });
  ...
  it("copies transaction addresses from the detail page", async () => {
    ...
    const originalClipboard = (
      navigator as Navigator & { clipboard?: { writeText: typeof writeText } }
    ).clipboard;
...
describe("transaction failures and types", () => {
  it("renders custom failure arguments as Name Type Data jq-style leaf rows", async () => {
    ...
    expect(
      screen.getByText("Complex(address,uint256,bool,(address,uint16),uint256[],uint256[][])"),
    ).toBeVisible();
    expect(screen.getByText("Revert data", { exact: true })).toBeVisible();
    const statusRow = screen
      .getByText("Status", { exact: true })
      .closest(".transaction-detail-row");
    const failureRow = screen
      .getByText("Failure reason", { exact: true })
      .closest(".transaction-detail-row");
        ...
        if (url.pathname === `/api/v1/transactions/${transactionHash}/failure`) {
          return envelope({
            ...
            decoding: {
              ...
              arguments: [
                { name: "message", type: "string", value: "insufficient balance", components: [] },
              ],
              candidates: [],
              abi_source: { kind: "builtin" },
            },
