...
sea_self_test=$("$docker_command" run --rm \
    ...
    "$image" \
    --self-test)
if [ "$sea_self_test" != '{"schema":"etherview-solcjs-sea-self-test-v1","sea":true,"node_version":"v26.9.0","wrapper_package":"solc@0.8.37","exec_argv":["--permission","--disable-sigusr1","--no-addons","--no-global-search-paths","--max-old-space-size=384"],"permissions":"restricted","write_denied":true}' ]; then
...
echo "docker-image-check: PASS (user=$configured_user, architecture=$image_architecture, SEA=Node-v26.9.0, Geas=0.3.3, Vyper=0.4.3, hardened rootfs)"
